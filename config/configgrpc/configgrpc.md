...
* `type ClientConfig struct {}`
  * gRPC client configuration's common settings
  * `endpoint`
    * target | exporter is going to send traces or metrics
      * -- via -- gRPC protocol
    * [valid syntax](https://github.com/grpc/grpc/blob/master/doc/naming.md)
  * `compression`
    * compression key for supported compression types | collector
  * `tls`
    * TLS struct exposes TLS client configuration
  * `keepalive`
    * // The keepalive parameters for gRPC client
    * [grpc.WithKeepaliveParams](https://godoc.org/google.golang.org/grpc#WithKeepaliveParams)
  * `read_buffer_size`
    * ReadBufferSize for gRPC client
    * [grpc.WithReadBufferSize](https://godoc.org/google.golang.org/grpc#WithReadBufferSize)
  * `write_buffer_size`
    * WriteBufferSize for gRPC
    * [grpc.WithWriteBufferSize](https://godoc.org/google.golang.org/grpc#WithWriteBufferSize)
  * `wait_for_ready`
    * // WaitForReady parameter configures client to wait for ready state before sending data.
    * [grpc.wait_for_ready](https://github.com/grpc/grpc/blob/master/doc/wait-for-ready.md)
  * `headers`
    * gRPC requests' headers
  * `balancer_name`
    * Sets the balancer in grpclb_policy to discover the servers
    * by default,
      * `pick_first`
    * [gRPC go](https://github.com/grpc/grpc-go/blob/master/examples/features/load_balancing/README.md)
  * `authority`
    * rewrite ":authority" header
    * [gRPC go](godoc.org/google.golang.org/grpc#WithAuthority)
  * `auth`
    * outgoing RPCs' auth configuration
  * `middlewares`
    * gRPC client's middlewares 
